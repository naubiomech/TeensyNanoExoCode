    ...
    if(_joint_data->is_used)
    {
        ...
        switch (exo_data->left_leg.hip.motor.motor_type)
        {
			...
			/* case (uint8_t)config_defs::motor::Maxon:
                ...
                #endif
                HipJoint::set_motor(new Maxon(id, exo_data, _Joint::get_motor_enable_pin(id, exo_data)));
                break; */
            default :
                //_motor = nullptr;
                #ifdef JOINT_DEBUG
   ...
   if(_joint_data->is_used)
   {
        ...
        switch (_data->left_leg.knee.motor.motor_type)
        {
			...
			case (uint8_t)config_defs::motor::Maxon:
                    ...
                    logger::println("Maxon");
                #endif
                KneeJoint::set_motor(new Maxon(id, exo_data, _Joint::get_motor_enable_pin(id, exo_data)));
                break;
            default :
                #ifdef JOINT_DEBUG
                    logger::println("NULL");
